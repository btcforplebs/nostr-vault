...
### Added
- **A Threaded Feed**: The feed's view control now cycles expanded -> condensed -> threaded instead of toggling two layouts. Threaded gathers a conversation — root note and every reply the feed is holding — into one card with an indented reply rail, folding past three replies so a long argument can't own the timeline. Tapping a line opens it in place into the full note with its action bar, so replying never leaves the timeline; tapping the open note goes to the thread — the same two-step gesture the condensed layout already had. It is stored per feed, so Following can stay expanded while Global runs threaded, and picking it turns the Replies filter on, because a threaded feed with replies hidden is just the layout you left.
- **One Density Control, Not Two**: The thread view had its own compact toggle, independent of the feed's. Density now lives on one axis: the feed decides how dense a timeline is, and a thread is always expanded, so a reply is one tap from wherever you land. Four condensed renderers became one; the thread view's compact toggle, its stored setting and three of those renderers are gone.
...
- **Two Taps to Reach the Reply Button**: Opening a conversation from the threaded feed landed you in the thread view's own compact mode, where the focused note drew as a condensed line with no action bar — the second tap only turned that mode off. Removing the thread view's compact mode fixes it at the root, and a tapped line in the feed now opens in place with its action bar, so reply is one tap from the timeline.
- **The Thread Toolbar Acted on the Wrong Note**: Toolbar Reply and Broadcast were bound to the note you arrived on while the note card in front of you was bound to the one you had focused, so after tapping into a reply the two reply buttons silently targeted different notes. Both now follow focus.
